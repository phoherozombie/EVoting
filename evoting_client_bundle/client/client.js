...
// ── GET /audit-status ─────────────────────────────────────────
app.get('/audit-status', async (_req, res) => {
    try {
        const upstream = await fetch(`${SERVER_URL}/audit-status`);
        if (!upstream.ok) {
            const text = await upstream.text();
            return res.status(upstream.status).json({ error: text });
        }
        ...
        console.error(`[${VOTER_ID}] /audit-status proxy error:`, err.message);
        res.status(502).json({ error: `Cannot reach tally server: ${err.message}` });
    }
});
...
app.post('/vote', async (req, res) => {
    ...
    try {
        ...
        // Generate nonce and commitment H(candidateIndex + ":" + nonce)
        const nonce = crypto.randomBytes(16).toString('hex');
        const commitment = crypto.createHash('sha256').update(candidateIndex + ':' + nonce).digest('hex');
        fs.writeFileSync(path.join(DATA, 'ballot_commitment.json'), JSON.stringify({
            candidateIndex,
            nonce,
            commitment
        }, null, 2));
        console.log(`[${VOTER_ID}] Generated ballot commitment: ${commitment}`);
        ...
        const data = await postBinary(`${SERVER_URL}/vote`, cipherBuf, {
            'Authorization': `Bearer ${authToken}`,
            'X-Ballot-Commitment': commitment
        });
...
// ── POST /client-reveal ───────────────────────────────────────
// Reads ballot_commitment.json and sends it anonymously to the server
app.post('/client-reveal', async (req, res) => {
    const commitmentFile = path.join(DATA, 'ballot_commitment.json');
    if (!fs.existsSync(commitmentFile)) {
        return res.status(404).json({ error: 'No ballot commitment found on this client.' });
    }
    try {
        const { candidateIndex, nonce } = JSON.parse(fs.readFileSync(commitmentFile, 'utf8'));
        console.log(`[${VOTER_ID}] Revealing ballot choice anonymously to server...`);
        
        const upstream = await fetch(`${SERVER_URL}/reveal-ballot`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ candidateIndex, nonce })
        });
        const data = await upstream.json();
        ...
        res.json({
            status: 'ok',
            message: 'Ballot choice anonymously revealed to the tally server.',
            server: data
        });
    } catch (err) {
        console.error(`[client-reveal] ERROR:`, err.message);
        res.status(500).json({ error: err.message });
    }
});
