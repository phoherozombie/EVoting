...
let commitmentRevealed     = false;  // true if this client has revealed ballot commitment
    ...
    try {
        ...
        switch (s.phase) {
            ...
            case 'complete':
                ...
                // Show audit panel in complete phase
                const auditPanel = document.getElementById('auditPanel');
                if (auditPanel) {
                    auditPanel.style.display = 'block';
                }
                
                // Auto-reveal once when entering complete phase
                if (!commitmentRevealed) {
                    revealBallotCommitment(true);
                }
                
                // Poll audit status periodically (every time we poll status)
                pollAuditStatus();
                break;
        }
...
// ── Anonymous Ballot Audit & Verification ──────────────────────
async function revealBallotCommitment(auto = false) {
    const btn = document.getElementById('btnRevealCommitment');
    if (btn) btn.disabled = true;
    
    if (!auto) {
        addLog('Initiating anonymous ballot audit and verification...', 'info');
    }
    
    try {
        const res = await fetch(CLIENT_BASE + '/client-reveal', { method: 'POST' });
        const data = await res.json();
        
        if (!res.ok) {
            // If it's a 404, it means this client didn't vote in this session (e.g. keygen only or not voted)
            if (res.status === 404) {
                if (!auto) {
                    addLog('Audit skipped: No ballot commitment found on this device.', 'warn');
                    alert('No ballot commitment found on this device. You must cast a vote first.');
                }
            } else {
                throw new Error(data.error || `HTTP ${res.status}`);
            }
            if (btn && !commitmentRevealed) btn.disabled = false;
            return;
        }
        
        commitmentRevealed = true;
        addLog('✓ Ballot choice and audit nonce revealed anonymously to server.', 'ok');
        if (btn) {
            btn.textContent = 'Ballot Verified ✅';
            btn.disabled = true;
        }
        
        // Refresh audit status immediately
        await pollAuditStatus();
        
    } catch (err) {
        console.error('Reveal error:', err);
        addLog(`Audit reveal failed: ${err.message}`, 'err');
        if (btn) {
            btn.disabled = false;
            btn.textContent = 'Verify My Ballot Anonymously';
        }
    }
}
...
async function pollAuditStatus() {
    try {
        const res = await fetch(CLIENT_BASE + '/audit-status');
        if (!res.ok) return;
        const s = await res.json();
        
        const badge = document.getElementById('auditStatusBadge');
        const totalCommitments = document.getElementById('auditTotalCommitments');
        const totalReveals = document.getElementById('auditTotalReveals');
        const detailsEl = document.getElementById('auditDetails');
        
        if (totalCommitments) totalCommitments.textContent = s.total_commitments || 0;
        if (totalReveals) totalReveals.textContent = s.total_reveals || 0;
        
        if (badge) {
            badge.textContent = `Audit: ${s.verification.toUpperCase()}`;
            badge.className = 'audit-badge badge'; // reset class
            
            if (s.verification === 'verified') {
                badge.style.borderColor = 'var(--success)';
                badge.style.color = 'var(--success)';
                badge.style.background = 'var(--success-bg)';
            } else if (s.verification === 'failed') {
                badge.style.borderColor = 'var(--error)';
                badge.style.color = 'var(--error)';
                badge.style.background = 'rgba(224,82,82,0.1)';
            } else {
                badge.style.borderColor = 'var(--gold-border)';
                badge.style.color = 'var(--gold)';
                badge.style.background = 'var(--gold-bg)';
            }
        }
        
        if (s.details && s.details.decryptedSum && detailsEl) {
            detailsEl.style.display = 'block';
            let html = '<div style="margin-bottom: var(--sp-2); font-weight: 600; color: var(--text-primary);">Audit Reconciliation Details:</div>';
            
            const candidatesList = candidates.length > 0 ? candidates : (s.details.candidates || []);
            candidatesList.forEach(cand => {
                const dec = s.details.decryptedSum[cand.id] || 0;
                const rev = s.details.revealedSum[cand.id] || 0;
                const status = dec === rev ? 'MATCH ✅' : 'MISMATCH ❌';
                html += `<div>Candidate[${cand.id}] (${cand.name}): Decrypted Tally = ${dec} | Audited Reveals = ${rev} | <span style="color: ${dec === rev ? 'var(--success)' : 'var(--error)'}">${status}</span></div>`;
            });
