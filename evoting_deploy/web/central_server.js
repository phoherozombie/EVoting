...
// OTP Store (in-memory)
// Format: { 'email@example.com': { hash: '...', expiresAt: 123456789, voter_id: '1' } }
...
const otpSendLimiter = rateLimit({
    ...
    message: { success: false, error: 'Vui lòng đợi 60 giây trước khi gửi lại mã OTP.' },
...
const otpVerifyLimiter = rateLimit({
    ...
    message: { success: false, error: 'Quá nhiều lần thử sai. Vui lòng thử lại sau 15 phút.' },
    standardHeaders: true,
    legacyHeaders: false,
});
...
app.get('/api/download/:filename', (req, res) => {
    let filepath;
    if (req.params.filename === 'crypto_params.bin') {
        filepath = path.join(BUILD_DIR, 'params/crypto_params.bin');
    } else {
        filepath = path.join(SERVER_DIR, req.params.filename);
    }
...
app.post('/api/register', (req, res) => {
    const { voter_id, password, email } = req.body;
    if (!voter_id || !password || !email) return res.status(400).json({ error: 'Thiếu thông tin (yêu cầu Voter ID, Password, và Email)' });
    ...
    voters.push({ voter_id, password, email });
...
app.post('/api/auth/send-otp', otpSendLimiter, async (req, res) => {
    ...
    let voters = JSON.parse(fs.readFileSync(VOTERS_FILE));
    const user = voters.find(v => v.email === email);

    // Luôn trả về success để chống enumeration attack
    if (!user) {
        return res.json({ success: true, message: 'Nếu email tồn tại trong hệ thống, mã OTP đã được gửi.' });
    }
    ...
    // Send email
    const mailOptions = {
        from: `"EVoting System" <${process.env.SMTP_EMAIL}>`,
        to: email,
        subject: 'Mã xác thực đăng nhập EVoting',
        text: `Mã OTP xác thực của bạn là: ${otpCode}. Mã này có hiệu lực trong 5 phút.`,
        html: `<p>Mã OTP xác thực của bạn là: <b>${otpCode}</b></p><p>Mã này có hiệu lực trong 5 phút. Vui lòng không chia sẻ mã này cho bất kỳ ai.</p>`
    };
...
app.post('/api/auth/verify-otp', otpVerifyLimiter, async (req, res) => {
    ...
    if (!email || !otp) return res.status(400).json({ error: 'Thiếu thông tin email hoặc OTP' });
    ...
    const isMatch = await bcrypt.compare(otp, storedOtpData.hash);
    
    if (isMatch) {
        // Xóa OTP sau khi sử dụng thành công
        otpStore.delete(email);

        const token = jwt.sign({ voter_id: storedOtpData.voter_id }, process.env.JWT_SECRET || 'super_secret_voting_key_2026', { expiresIn: '1h' });
        res.json({ success: true, token, voter_id: storedOtpData.voter_id });
    } else {
        res.status(401).json({ error: 'Mã OTP không chính xác' });
    }
});
...
app.post('/api/upload', authenticateJWT, upload.single('file'), (req, res) => {
    ...
    else if (type === 'pk_share') {
        const id = parseInt(req.body.id, 10);
        const share1 = path.join(SERVER_DIR, 'pk_share1.bin');
        const share2 = path.join(SERVER_DIR, 'pk_share2.bin');
        const share3 = path.join(SERVER_DIR, 'pk_share3.bin');
        
        if (fs.existsSync(share1) && fs.existsSync(share2) && fs.existsSync(share3)) {
            console.log(`[Server] Đã đủ 3 mảnh khóa! Tự động gộp khóa (aggregate)...`);
            try {
                execSync('./server_key_aggregate', { cwd: BUILD_DIR, stdio: 'inherit' });
                console.log(`[Server] Gộp khóa thành công (joint_pk.bin).`);
            } catch (err) {
                console.error('Lỗi gộp khóa:', err.message);
            }
        }
        res.json({ success: true, message: `Nhận khóa phần của Voter ${id} thành công` });
    }
});
